...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private String saveCrashInfo2File( Throwable ex ) {
        ...
        sb.append( result );
        if ( Debug ) {
            Log.e( "CrashHandler", "saveCrashInfo2File: \n" + sb.toString() );
        }
        // 保存文件
        ...
        if ( Environment.getExternalStorageState().equals( Environment.MEDIA_MOUNTED ) ) {
            try {
                File dir = FileUtils.CRASHFILE;
                if ( Debug ) {
                    Log.e( "CrashHandler", "saveCrashInfo2File: " + dir.toString() );
                }
                Toast.makeText( mContext, "CrashHandler" + dir.toString(), Toast.LENGTH_SHORT ).show();
