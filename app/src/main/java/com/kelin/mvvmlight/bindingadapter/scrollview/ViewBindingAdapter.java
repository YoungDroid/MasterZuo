...
public final class ViewBindingAdapter {
    ...
    @BindingAdapter({ "onScrollCompletePercent", "onScrollCompleteCommand" })
    public static void onScrollCompleteCommand( final NestedScrollView nestedScrollView, double completePercent, final ReplyCommand onCompleteCommand ) {
        nestedScrollView.setOnScrollChangeListener( ( OnScrollChangeListener ) ( v, scrollX, scrollY, oldScrollX, oldScrollY ) -> {

            if ( v.getChildCount() > 0 ) {
                float percent = ((float ) (scrollY + v.getHeight())) / v.getChildAt( 0 ).getHeight();
                if ( onCompleteCommand != null && percent >= completePercent ) {
